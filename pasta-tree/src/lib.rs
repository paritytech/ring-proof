...
mod tests {
    ...
    fn _bench_msm<C: CurveGroup>(log_n: u32) {
        ...
        let (scalars, bases): (Vec<_>, Vec<_>) = (0..n).map(|_| (C::ScalarField::rand(rng), C::Affine::rand(rng))).unzip();
        let t_msm = start_timer!(|| format!("log(n)={log_n}, MSM on {}", ark_std::any::type_name::<C::Config>()));
    ...
    fn mul_endo_wnaf(p: ark_pallas::Projective, k1: (bool, ark_pallas::Fr), k2: (bool, ark_pallas::Fr)) -> ark_pallas::Projective {
        let mut p1 = p;
    ...
    fn _bench_folding(log_n: u32) {
        let rng = &mut test_rng();
        let n = 2usize.pow(log_n);
        let (l, r): (Vec<ark_pallas::Affine>, Vec<ark_pallas::Affine>) = (0..n).map(|_| (ark_pallas::Affine::rand(rng), ark_pallas::Affine::rand(rng))).unzip();
        let x = ark_pallas::Fr::rand(rng);
        let _timer = start_timer!(|| format!("Naive folding, log(n) = {log_n}"));
        let res: Vec<ark_pallas::Projective> = ark_std::cfg_iter!(l).zip(r.clone())
            .map(|(l, r)| r * x + l)
            .collect();
        end_timer!(_timer);

        let _timer = start_timer!(|| format!("Naive folding with endo, log(n) = {log_n}"));
        let res_: Vec<ark_pallas::Projective> = ark_std::cfg_into_iter!(l.clone()).zip(ark_std::cfg_into_iter!(r.clone()))
            .map(|(l, r)| l + <PallasConfig as GLVConfig>::glv_mul_affine(r, x))
            .collect();
        end_timer!(_timer);
        assert_eq!(res_, res);

        let _timer = start_timer!(|| format!("Naive folding with endo and w-NAF, log(n) = {log_n}"));
        let ((sgn_k1, k1), (sgn_k2, k2)) = PallasConfig::scalar_decomposition(x);
        let res_: Vec<ark_pallas::Projective> = ark_std::cfg_into_iter!(l).zip(ark_std::cfg_iter!(r))
    ...
    fn batch_double_affine<C: GLVConfig>(bases: Vec<Affine<C>>) -> Vec<Affine<C>> {
        let mut denoms: Vec<C::BaseField> = ark_std::cfg_iter!(bases)
            .map(|p| p.y + p.y)
            .collect();

        ark_ff::batch_inversion(&mut denoms);

        ark_std::cfg_iter!(bases).zip(denoms).map(|(p, _2y_inv)| {
            let (x, y) = p.xy().unwrap();
            let t = _2y_inv * (x.square() * C::BaseField::from(3) + <C as SWCurveConfig>::COEFF_A); // (3x^2 + a) / 2y
    ...
    fn batch_double_affine_in_place<C: GLVConfig>(bases: &mut [Affine<C>]) {
        ...
        cfg_iter_mut!(bases).zip(ark_std::cfg_into_iter!(denoms))
            .for_each(|(p, _2y_inv)| {
    ...
    fn batch_add_affine<C: GLVConfig>(bases1: Vec<Affine<C>>, bases2: Vec<Affine<C>>) -> Vec<Affine<C>> {
    ...
    fn batch_mul_by_x_affine<C: GLVConfig>(bases: Vec<Affine<C>>, x: C::ScalarField) -> Vec<Affine<C>> {
    ...
    fn bench_folding() {
        ...
        let x = ark_pallas::Fr::rand(rng);
        let dbl: Vec<_> = bases.iter().map(|p| {
        ...
        let add: Vec<_> = bases.into_iter().zip(bases2.iter())
            .map(|(p1, p2)| p1 + p2)
            .collect();
        ...
        let _timer = start_timer!(|| format!("Naive folding, log(n) = {log_n}"));
        let res_: Vec<ark_pallas::Projective> = ark_std::cfg_into_iter!(bases1).zip(ark_std::cfg_into_iter!(bases2))
            .map(|(l, r)| l + r * x)
            .collect();
