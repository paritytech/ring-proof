...
struct CycleSideParams<C: CurveGroup, G: SWCurveConfig<BaseField = C::ScalarField>> {
    ...
    piop_params: PiopParams<Affine<G>>,
}

struct CycleParams<
    C: SWCurveConfig,
    G: SWCurveConfig<BaseField = C::ScalarField, ScalarField = C::BaseField>,
> {
    c0_params: CycleSideParams<Projective<C>, G>,
    c1_params: CycleSideParams<Projective<G>, C>,
}

impl<C: SWCurveConfig, G: SWCurveConfig<BaseField = C::ScalarField, ScalarField = C::BaseField>>
    CycleParams<C, G>
where
    C::BaseField: PrimeField,
{
    pub fn setup<R: Rng>(rng: &mut R, domain_size: usize) -> Self {
        ...
        let piop_params_0 =
            PiopParams::<Affine<G>>::setup(domain_0, pcs_params_1.h, seed_0, padding_0);
        let seed_1 = Affine::<C>::rand(rng);
        let padding_1 = Affine::<C>::rand(rng);
        let piop_params_1 =
            PiopParams::<Affine<C>>::setup(domain_1, pcs_params_0.h, seed_1, padding_1);
        Self {
            c0_params: CycleSideParams {
                pcs_params: pcs_params_0,
                piop_params: piop_params_0,
            },
            c1_params: CycleSideParams {
                pcs_params: pcs_params_1,
                piop_params: piop_params_1,
            },
        }
    }
}
...
mod tests {
    ...
    fn random_node<C: CurveGroup, G: SWCurveConfig<BaseField = C::ScalarField>, R: Rng>(
        params: &CycleSideParams<C, G>,
        child: Option<Affine<G>>,
        rng: &mut R,
        ...
        let parent = params
            .pcs_params
            .commit_hiding(children_xs, C::ScalarField::zero());
        ...
        C1: SWCurveConfig<BaseField = F0, ScalarField = F1>,
    {
        ...
        let (l2_column, l2_vk) =
            c1_params.commit_child_nodes(l2_nodes.siblings.as_slice(), l2_nodes_parent_r);
